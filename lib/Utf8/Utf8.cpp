...
bool utf8IsNoBreakBeforeCjkPunctuation(const uint32_t cp) {
  switch (cp) {
    case '.':
    case ',':
    case ':':
...
bool utf8IsNoBreakAfterCjkPunctuation(const uint32_t cp) {
  switch (cp) {
    case '(':
    case '[':
    case '{':
...
bool utf8HasCjkBreakOpportunityBetween(const uint32_t leftCp, const uint32_t rightCp) {
  ...
  if (utf8IsNoBreakAfterCjkPunctuation(leftCp) || utf8IsNoBreakBeforeCjkPunctuation(rightCp)) return false;
