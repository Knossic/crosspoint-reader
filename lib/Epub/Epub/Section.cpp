...
// v46: Drop whitespace between CJK characters except Hangul.
...
int Section::buildProgressPercent() const {
  if (buildComplete_) return 100;
  if (!build_ || !build_->parser || build_->totalBytes == 0) return 0;
  return static_cast<int>(
      std::min<uint64_t>(99, static_cast<uint64_t>(build_->parser->parseBytesConsumed()) * 100 / build_->totalBytes));
}
