...
int GfxRenderer::resolveTextFontId(const int fontId, const char* text, const EpdFontFamily::Style style) const {
  ...
  while ((cp = utf8NextCodepoint(reinterpret_cast<const uint8_t**>(&cursor)))) {
    if (&fontFallback::select(primary, &fallback, cp, style) == &fallback) {
...
int GfxRenderer::getTextWidth(const int fontId, const char* text, const EpdFontFamily::Style style,
  ...
  // Resolve the fallback for preparation; glyph selection keeps primary coverage.
  const int resolvedFontId = resolveTextFontId(fontId, text, style);
  ...
  if (resolvedFontId != fontId) {
    ...
    return fontFallback::measure(fontMap.at(fontId), fontIt->second, renderedText, style).width;
  }
...
void GfxRenderer::drawText(const int fontId, const int x, const int y, const char* text, const bool black,
  ...
  // Find a fallback to prewarm; supported characters keep the requested font.
  const int resolvedFontId = resolveTextFontId(fontId, text, style);
  ...
  const EpdFontFamily* fallback = resolvedFontId != fontId ? &fontMap.at(resolvedFontId) : nullptr;
  ...
  if (fontCacheManager_ && fontCacheManager_->isScanning()) {
    if (!fallback) {
      ...
      const auto* cursor = reinterpret_cast<const unsigned char*>(renderedText);
      while (*cursor) {
        ...
        encoded[length] = '\0';
        const auto& selected = fontFallback::select(font, fallback, cp, style);
        fontCacheManager_->recordText(encoded, &selected == &font ? fontId : resolvedFontId, style);
      }
    }
    return;
  }
  ...
  while ((cp = utf8NextCodepoint(reinterpret_cast<const uint8_t**>(&textCursor)))) {
    ...
    if (utf8IsCombiningMark(cp) || BidiUtils::isTransparentMark(cp)) {
      ...
      renderCharImpl<TextRotation::None>(*this, renderMode, font, cp, combiningX, lastBaseBaselineY - raiseBy, black,
                                         style);
      continue;
    }

    const auto& glyphFont = fontFallback::select(font, fallback, cp, style);
    cp = glyphFont.applyLigatures(cp, textCursor, style);
    ...
    if (prevCp != 0) {
      const auto kernFP =
          previousFont == &glyphFont ? glyphFont.getKerning(prevCp, cp, style) : 0;  // 4.4 fixed-point kern
    ...
    const int glyphY = &glyphFont == &font ? yPos
                                           : y + getFontAscenderSize(resolvedFontId) +
                                                 (getLineHeight(fontId) - getLineHeight(resolvedFontId)) / 2;
    if (isSupSub) {
      // yPos already carries the vertical offset applied by TextBlock::render().
      renderCharScaled(*this, renderMode, glyphFont, cp, lastBaseX, glyphY, black, style);
    } else {
      renderCharImpl<TextRotation::None>(*this, renderMode, glyphFont, cp, lastBaseX, glyphY, black, style);
    }
    lastBaseBaselineY = glyphY;
    prevCp = cp;
    previousFont = &glyphFont;
  }
}
...
int GfxRenderer::getTextAdvanceX(const int fontId, const char* text, EpdFontFamily::Style style) const {
  // Match drawText's per-glyph fallback for mixed UI strings.
  const int resolvedFontId = resolveTextFontId(fontId, text, style);
  ...
  if (resolvedFontId != fontId) {
    ...
    return fontFallback::measure(fontMap.at(fontId), fontMap.at(resolvedFontId), text, style).advance;
  }
...
void GfxRenderer::drawTextRotated90CW(const int fontId, const int x, const int y, const char* text, const bool black,
  ...
  // Prepare fallback glyphs while retaining primary-font coverage.
  const int resolvedFontId = resolveTextFontId(fontId, text, style);
  // Redirected to the SD fallback: batch-load the string's glyphs so the draw
  // loop below doesn't fault them in one SD read at a time (#2725).
  if (resolvedFontId != fontId) {
  ...
  while ((cp = utf8NextCodepoint(reinterpret_cast<const uint8_t**>(&text)))) {
    ...
    if (utf8IsCombiningMark(cp) || BidiUtils::isTransparentMark(cp)) {
      ...
      const int raiseBy =
          combiningMark::raiseAboveBase(anchor, combiningGlyph->top, combiningGlyph->height, lastBaseTop);
      const int combiningX = lastBaseBaselineX - raiseBy;
    ...
    const auto& glyphFont = fontFallback::select(font, fallback, cp, style);
    cp = glyphFont.applyLigatures(cp, text, style);
    ...
    const int glyphX = &glyphFont == &font ? x : x + (getLineHeight(fontId) - getLineHeight(resolvedFontId)) / 2;
    renderCharImpl<TextRotation::Rotated90CW>(*this, renderMode, glyphFont, cp, glyphX, lastBaseY, black, style);
    lastBaseBaselineX = glyphX + glyphFont.getData(style)->ascender - font.getData(style)->ascender;
    prevCp = cp;
    previousFont = &glyphFont;
  }
}
