...
class GfxRenderer {
 ...
 private:
  ...
  // UI font ID -> SD fallback. Primary glyphs keep their font and metrics.
  std::map<int, int> fallbackFontMap_;

  // Returns the fallback ID when it covers a missing non-combining glyph;
  // otherwise returns fontId. Used for batch preparation, not whole-string routing.
  ...
  // found missing glyphs covered by the SD fallback: UI screens (file browser, home)
