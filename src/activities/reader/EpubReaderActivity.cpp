...
bool EpubReaderActivity::buildProgressUpdateDue() const {
  return section && millis() - buildPopupLastMs >= 2000 && section->buildProgressPercent() >= buildPopupLastPercent + 5;
}

void EpubReaderActivity::showBuildProgress(const bool force) {
  if (!renderer.hasFrameBuffer() || (!force && !buildProgressUpdateDue())) return;
  const int percent = section ? section->buildProgressPercent() : 0;
  char message[32];
  snprintf(message, sizeof(message), tr(STR_INDEXING_PERCENT), percent);
  GUI.drawPopup(renderer, message);
  pagesUntilFullRefresh = 1;
  buildPopupLastPercent = percent;
  buildPopupLastMs = millis();
}
...
void EpubReaderActivity::renderBook() {
    ...
    if (!cacheComplete) {
      ...
      if (needsFullBuild) {
        ...
        const auto popupFn = [this]() {
          if (renderer.hasFrameBuffer()) showBuildProgress();
        };
        std::optional<GfxRenderer::FrameBufferLoan> loan;
        loan.emplace(renderer);
        bool built = section->startBuild(renderSpec, popupFn);
        while (built && !section->isBuildComplete()) {
          built = section->buildSomeMore(BUILD_PAGES_PER_CHUNK);
          if (built && !section->isBuildComplete() && buildProgressUpdateDue()) {
            loan.reset();
            showBuildProgress();
            loan.emplace(renderer);
          }
        ...
        if (section->isPartial() &&
          ...
          if (anchorJump) {
            ...
            showPopup = !targetAvailable &&
                        (offsetJump.has_value() || (spineBytes > BUILD_POPUP_BYTE_THRESHOLD && willInflate) ||
                         target > BUILD_POPUP_PAGE_THRESHOLD);
          }
          if (showPopup) showBuildProgress(true);
          ...
          while (!section->isBuildComplete() &&
            ...
            if (!buildPopupPending) showBuildProgress();
  ...
  if (section->isBuilding()) {
    if (section->currentPage >= static_cast<int>(section->pageCount)) showBuildProgress(true);
    while (!section->isBuildComplete() && section->currentPage >= static_cast<int>(section->pageCount)) {
      showBuildProgress();
      if (!section->buildSomeMore(BUILD_PAGES_PER_CHUNK)) {
        LOG_ERR("ERS", "Failed during incremental section build");
